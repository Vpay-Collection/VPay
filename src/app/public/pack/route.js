(function (global, document, $) {
     ...
     *   }}
     */
    function getRouter(route, isNew) {
...
//此处预检
                mdbAdmin.autoLoadComponents($htmlDom, function () {
                    ...
                    mdbAdmin.initComponents(container, function () {
                        ...
                        if (typeof handler.onrender === "function") {
                            handler.onrender(queryParams, $dom, data);
                        }
                        hideLoading();
                    });
                });
    ...
    function routerGo(event) {
        ...
        var newHandler = getRouter(newHash, true);
        ...
        if (newHandler) {
            ...
            if (newHandler.libs) {
                resourceLoader.use(newHandler.libs, function () {
                    libsDefer.resolve();
                });
            } else {
                libsDefer.resolve();
            ...
            log.danger("找不到路由地址：" + event.newURL);
            go("error");
        }
    }
