...
(function (global, document, $) {
	...
	var tables = {
		...
		plugin: {
			...
			"[data-mdb-sortable]": "drag-and-drop",
			"[data-mdb-countdown]": "countdown",
			".ecommerce-gallery": "ecommerce-gallery",
			".dummy": "dummy",
			"[data-mdb-auto-filter]": "filters",
	...
	class Loading {
		...
		constructor(parent, msg) {
			...
			loadingIndex++;
			const loader = `
    <div class="loading loading-delay ankio-loading " style="z-index: ${loadingIndex}"  data-mdb-loader="${this.id}" data-mdb-backdrop-id="backdrop-${this.id}" data-mdb-scroll="false">
						...
						function callback(func) {
							try {
								...
								if ($.isFunction(func.getOrCreateInstance)) {
									let result = func.getOrCreateInstance(vv, option);
									if ($.isFunction(result.update)) {
										result.update(option);
									}
								} else if ($.isFunction(func.getInstance)) {
									let data = func.getInstance(vv);
									if (!data) {
										data = new func(vv, option);
										if ($.isFunction(data.init)) {
											data.init();
										}
									}
