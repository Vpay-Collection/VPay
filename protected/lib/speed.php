...
class Model{
    ...
    public function reset($table_name){if($table_name)$this->table_name = $table_name;}
  ...
  //增加一种
    public function insert_duplicate($row, $colums)
    {//若重复插入，则进行更新
        $values = array();
        foreach($row as $k=>$v){
            $keys[] = "`{$k}`"; $values[":".$k] = $v; $marks[] = ":".$k;
    ...
    public function insert_ignore($row)
    {//若重复插入，则忽略重复值
        $values = array();
        foreach ($row as $k => $v) {
            $keys[] = "`{$k}`";
    ...
    public function findSum($conditions, $parm)
    {//对某个字段进行求和
        $conditions = $this->_where($conditions);
        $count = $this->query("SELECT SUM($parm) AS M_COUNTER FROM " . $this->table_name . $conditions["_where"], $conditions["_bindParams"]);
        return isset($count[0]['M_COUNTER']) && $count[0]['M_COUNTER'] ? $count[0]['M_COUNTER'] : 0;
    }
